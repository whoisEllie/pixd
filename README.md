...
- **Palette**: a curated, hand-maintained block list with real textures, searchable.
  Edit [`src/data/blocks.ts`](src/data/blocks.ts) to add/remove blocks or swap textures.
...
grid run-length encoded with the `voxel-crunch` scheme. Block ids come from the curated
palette in `src/data/blocks.ts` and must match bloxd's `blockMetadata`.
...
```
src/
  data/blocks.ts           curated, hand-maintained block palette (edit this)
  assets/textures/         bundled block textures
  schem/                   encoding core (RLE, Avro, schematic builder, palette, textures)
